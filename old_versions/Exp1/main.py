...
from evaluation import sample_periodically, sample_periodically_old, sample_periodically_full_test
import shutil
...
def main(args):
    ...
    run_ncp = params['run_ncp']
    max_it = params['max_it']
    epochs = 1
    num_sched_steps_per_epoch = max_it // params['sched_lr_update']
    num_sched_steps_per_epoch_ncp = max_it // params['sched_lr_update_ncp']
    lr = params['lr']
    min_lr = params['min_lr']
    lr_ncp = params['lr_ncp']
    min_lr_ncp = params['min_lr_ncp']
    ...
    if run_ncp:
        dpmm_ncp = NeuralClustering_old(params).to(params['device'])
    ...
    if run_ncp:
        optimizer_old = torch.optim.Adam(dpmm_ncp.parameters() , lr=lr_ncp, weight_decay=weight_decay)
    ...
    lr_schedule_values_ncp = cosine_scheduler(
        lr_ncp, min_lr_ncp, epochs, num_sched_steps_per_epoch_ncp,
        warmup_epochs=0, warmup_steps=0,)
    
    print('lr_schedule_values size:', len(lr_schedule_values))
    print('lr_schedule_values_ncp size:', len(lr_schedule_values_ncp))
    ...
    stats = {'NMI_sum': 0, 'ARI_sum': 0, 'nll_sum': 0, 'count': 0, 
             'NMI_max': 0, 'ARI_max': 0, 'NMI_max_it': 0, 'ARI_max_it': 0,
             'NMI_full_test_max': 0, 'ARI_full_test_max': 0, 'NMI_full_test_max_it': 0, 'ARI_full_test_max_it': 0}
    stats_ncp = {'NMI_ncp_sum': 0, 'nll_ncp_sum': 0, 'count_ncp': 0, 'NMI_ncp_max': 0, 'NMI_ncp_max_it': 0}
    
    while it < max_it:
                
        dpmm.train()  # new model
        if run_ncp:
            dpmm_ncp.train()  # old model (NCP from Ari's paper)
        ...
        # Update learning rate & weight decay (NCP):
        if run_ncp and (lr_schedule_values_ncp is not None or wd_schedule_values is not None) and it % params['sched_lr_update_ncp'] == 0:
            for i, param_group in enumerate(optimizer_old.param_groups):
                if lr_schedule_values_ncp is not None:
                    param_group["lr"] = lr_schedule_values_ncp[it_lr_sched_ncp]
                if wd_schedule_values is not None and param_group["weight_decay"] > 0:
                    param_group["weight_decay"] = wd_schedule_values[it_lr_sched_ncp]
            
            it_lr_sched_ncp = it_lr_sched_ncp + 1
        ...
        if it % plot_freq == 0 or it == 1:
            ...
            stats = sample_periodically(wnb, data, cs_gt, params, dpmm, it, N_sampling, show_histogram, stats)
            if run_ncp:
                stats_ncp = sample_periodically_old(wnb, data, cs_gt, params, dpmm_ncp, it, N_sampling, show_histogram, stats_ncp)
            
            # # Test the model on the whole tests data:
            # data, cs_gt = data_generator.get_full_test_data() 
            # stats = sample_periodically_full_test(wnb, data, cs_gt, params, dpmm, it, stats)
        ...
        if run_ncp: 
            curr_lr_ncp = optimizer_old.param_groups[0]['lr']
        # print(optimizer.param_groups[0]['lr'])
            
        if not run_ncp: 
            print('Iteration: {0} N: {1} Current lr: {2:.10f}'.format(it, N, curr_lr))
        else:
            print('Iteration: {0} N: {1} Current lr: {2:.10f} Current lr_ncp: {3:.10f}'.format(it, N, curr_lr, curr_lr_ncp))
        ...
        for n in range(1, N):  # n is the next point to be assigned. Points up to n-1 (in each point in the batch, so there are B * (n-1)) were already assigned.                              
            ...
            # -------- KL LOSS - OLD MODEL --------
            if run_ncp:
                logprobs_kl_old  = dpmm_ncp(data, cs, n) # Shape: [B, K'+1]. This is the log probabilities of assigning the n-th point to the K' clusters we revealed so far, or to a new cluster.               
        ...
        if run_ncp:
            NCP_NMI_train = compute_NMI(cs, cs_train_old, None)                
        ...
        if not run_ncp:
            ...
            sts = update_stats_train_ncp(it, N, K, loss, kl_loss, mc_loss, j_loss, entrpy, NMI_train, NCP_NMI_train, loss_old)  # stats.update({'train_acc1': acc_train})
    ...
    # Print avg metrics:
    print('\n * Average NMI: {0:.3f}, Average ARI: {1:.3f}, Max NMI (full test): {2:.3f} (on iter:) {3:.3f}, Max ARI (full test): {4:.3f} (on iter:) {5:.3f}'.format(stats['NMI_sum']/stats['count'], stats['ARI_sum']/stats['count'], stats['NMI_full_test_max'], stats['NMI_full_test_max_it'], stats['ARI_full_test_max'], stats['ARI_full_test_max_it'])) 
    print('\n')
    if run_ncp:
        print('\n * Average nll (NCP): {0:.3f}, Average NMI (NCP): {1:.3f}, Max NMI (NCP): {2:.3f} (on iter:) {3:.3f}'.format(stats_ncp['nll_ncp_sum']/stats_ncp['count_ncp'], stats_ncp['NMI_ncp_sum']/stats_ncp['count_ncp'], stats_ncp['NMI_ncp_max'], stats_ncp['NMI_ncp_max_it'])) 
        print('\n')
