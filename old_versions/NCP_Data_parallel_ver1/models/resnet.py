...
class ResNet(nn.Module):
    ...
    def forward(self, x, it):
        # x is in shape [batch_sz, nc * img_sz * img_sz]
        x = torch.reshape(x, (x.shape[0], self.img_nc, self.img_sz, self.img_sz))
        
        out = F.relu(self.bn1(self.conv1(x)))
        out, it = self.layer1(out, it)
        out, it = self.layer2(out, it)
        out, it = self.layer3(out, it)
        out, it = self.layer4(out, it)
...
def resnet18(params):
    return ResNet(params, BasicBlock, [2, 2, 2, 2], num_classes=params['nlabels'])


def resnet34(params):
    return ResNet(params, BasicBlock, [3, 4, 6, 3], num_classes=params['nlabels'])
