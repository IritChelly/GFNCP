...
def main(args):
    ...
    dpmm = torch.nn.DataParallel(net, device_ids=[0,1]).to(torch.device('cuda'))
    ...
    while it < max_it:
        ...
        for n in range(1, N):  # n is the next point to be assigned. Points up to n-1 (in each point in the batch, so there are B * (n-1)) were already assigned.                              
            E, E_mask = dpmm(data, cs, n)   # E is [B, K + 1]. This is the unnormalized logprob of p(c_{0:n} | x).
            ...
            # # -------- J Loss: --------------------
            if n == N - 1:
                j_loss, data_E, entrpy = j_loss_func(dpmm, E, N, n, data, cs[0, :], it)  # scalar
            # # -------- END J Loss: ----------------
            
            
            # -------- MC Loss: --------------------
            mc_n_term, log_pn = mc_loss_func(E, E_mask, log_pn, n, N, cs[0, :], batch_size, it, device, epsilon=1e-5)
            mc_loss += mc_n_term
            # -------- END MC Loss: ----------------
        ...
        NMI_train = compute_NMI(cs[0, :], cs_train, None)
        ARI_train = compute_ARI(cs[0, :], cs_train, None)           
