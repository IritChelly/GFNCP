import numpy as np
from ncp_prob_computation_old import NCP_prob_computation_old
import torch
...
def histogram_for_data_perms_old(data, dpmm, params, N=20, perms=100, is_old_ncp=False):
    '''
    ...
    css, probs, _ = ncp_sampler.sample(S) # css (cs test): [S, N]; probs: [S,]
    ...
    prob_computer = NCP_prob_computation_old(dpmm, data)
    ...
    for p in range(perms + 1):
        ...
        prob_computer = NCP_prob_computation_old(dpmm, data_perm)
    ...
    counts, bins = np.histogram(probs_for_histogram, bins=20)
    # print(counts, bins)
...
def plot_best_clustering_old(dataname, data, css, prob_css):
    '''
