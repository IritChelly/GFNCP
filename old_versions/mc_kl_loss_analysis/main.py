...
def main(args):
    ...
    while it < max_it:
        ...
        if it % plot_freq == 0 or it == 1:
            ...
            if N_sampling > 20:
                ...
                N_tmp = N_sampling
            data, cs_gt, clusters, K = data_generator.generate(N=N_tmp, batch_size=1, train=False)  # data: [1, N, 2] or [1, N, 28, 28] or [1, N, 3, 28, 28]            
            plot_samples_and_histogram(wnb, data, cs_gt, params, dpmm, it, N=N_tmp, show_histogram=show_histogram)
    ...
    # Print avg metrics:
    print('\n * Max NMI (test): {0:.3f} (on iter:) {1:.3f}, Max ARI (test): {2:.3f} (on iter:) {3:.3f}'.format(stats['NMI_max'], stats['NMI_max_it'], stats['ARI_max'], stats['ARI_max_it'])) 
    print('\n')
