...
def kl_loss_func(E, E_mask):
    ...
    A = ((-E + m) * E_mask)     
    S = torch.log((torch.exp(-E + m) * E_mask).sum(dim=1, keepdim=True))
    print('A(n):', A[0, :])
    print('S:', S[0, :])
    print('A(n) - S:', (A - S)[0, :])
    logprobs_kl = A - S
...
def j_loss_func(dpmm, E, N, n, cs, it):
...
def mc_loss_func(E, E_mask, log_pn, n, N, cs, batch_size, it, device, epsilon=1e-5):
    MC_n_term = torch.zeros(1).to(device)
    if n == 1:
        m, _ = torch.min(E, 1, keepdim=True)    # [B, 1] 
        ...
        A = torch.log(epsilon + in_edge)
        S = torch.log(epsilon + out_edges_sum)
        print('A(n-1):', A[0, :])
        print('S:', S[0, :])
        print('A(n-1) - S:', (A - S)[0, :])
        ...
        if n == N - 1:
            ...
            reward = torch.tensor(10000).repeat(batch_size).to(device)  # [B,]. Fixed value used in the second term of the MC objective.
            
            A = torch.log(epsilon + in_edge_last)
            S = torch.log(epsilon + reward)
            print('A(n-1):', A[0, :])
            print('S:', S[0])
            print('A(n-1) - S_N:', (A - S)[0, :])
